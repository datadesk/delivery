...
export class Delivery extends EventEmitter {
  ...
  async downloadFile(
    ...
    if (!isIdentical) {
      ...
      await outputFile(dest, response.Body as Readable);
    }
  ...
  async downloadFiles(prefix: string, dir: string) {
    ...
    for await (const content of this.listObjects(prefix)) {
      if (
        // unclear why the Key would ever be empty, but TS says it might be
        content.Key == null ||
        // the endsWith('/') + size check for folders is good enough for aws-cli, so it's good enough for us
        // reference: https://github.com/aws/aws-cli/blob/83b43782dd174a9766b3e7870a3a5aade074dc3c/awscli/customizations/s3/filegenerator.py#L326
        (content.Key.endsWith('/') && content.Size === 0)
      ) {
        continue;
      }

      const Key = relative(this.basePath, content.Key);
      const s3ETag = content.ETag;
  ...
  async getS3ObjectETag(Key: string) {
    ...
    try {
      const { ETag } = await this.s3.send(new HeadObjectCommand(params));
  ...
  isFileIdenticaltoS3File(
    ...
    // the AWS API returns this with extra quotes for some reason, so make
  ...
  /**
   *  A tiny wrapper around `paginateListObjectsV2` that allows us to ignore
   *  how it returns objects in pages, instead yielding each individual object
   *  as needed.
   *
   * @private
  ...
  async *listObjects(prefix: string) {
    ...
    const input: ListObjectsV2CommandInput = { Bucket: this.bucket, Prefix };

    for await (const page of paginateListObjectsV2(config, input)) {
